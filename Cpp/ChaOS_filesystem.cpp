...
{
	char_short temp; temp.USHORT = disk.diskSize; // short na char(diskSize)
	char_int temp2; temp2.INT = (~0); // int na bitVector

	bitMask m;
	temp2.CHAR[0] = m.setBit(temp2.CHAR[0], 0, 0); // oznaczamy zajęty sektor
	...
	if (rootDirSector == 0) { /*wyjątek brak miejsca */ }
...
c_uShort ChaOS_filesystem::allocateSector()
{
	...
	{
		if (m.getBit(freeSectorBitVector, i))
		{
			freeSectorBitVector = m.setBit(freeSectorBitVector, i, 0);
			freeSectorCount--;

			VCB[8] = freeSectorCount;
			temp.INT = freeSectorBitVector;
			VCB[16] = temp.CHAR[3]; VCB[17] = temp.CHAR[2]; VCB[18] = temp.CHAR[1]; VCB[19] = temp.CHAR[0];
...
void ChaOS_filesystem::freeSector(uShort number)
{
	...
	char_int temp;
	temp.CHAR[3] = VCB[16]; temp.CHAR[2] = VCB[17]; temp.CHAR[1] = VCB[18]; temp.CHAR[0] = VCB[19];//char[4] to int

	freeSectorBitVector = temp.INT;

	bitMask m;
	freeSectorBitVector = m.setBit(freeSectorBitVector, number, 1);
	freeSectorCount++;
...
void ChaOS_filesystem::create(const char * f, type t)
{
	...
	bool exist = search(name, type::dir) || search(name, type::file);
	if (exist)
	{
		throw objectExists();
	}
	...
	char row[8] = { name[0], name[1], name[2], name[3], name[4], 0, 1, char(t) }; //nowy wpis
	...
	while (!fileCreated)
	{
		...
		for (int i = 0, j = 0; j < 3 && !fileCreated; i += 8, j++)
		{
			if (dirSector[i] == 0)
			{
				...
				char fileSector[32] = {};
				disk.writeSector(dirSector[i + 5], fileSector);
...
void ChaOS_filesystem::remove(const char * f)
{
	...
	bool exist = search(name, type::file) || search(name, type::dir);
	if (!exist)
	{
...
std::string ChaOS_filesystem::listDirectory()
{
	...
	do
	{
		for (int i = 0, j = 0; i < 3; i++, j += 8)
		{
			if (dirSector[j] != 0)
			{
				...
				result << "\t " << (unsigned int)dirSector[5 + j] << "\t " << (unsigned int)dirSector[6 + j] << "\t " << dirSector[7 + j] << std::endl;
			}
		}
		char next = dirSector[31];
		if (next != 0)
		{
			disk.readSector(next, dirSector);
		}
		else
		{
			end = true;
	...
	return result.str(); // zwracany obiekt (?)
}
...
void ChaOS_filesystem::openFile(const char * filename)
{
	...
	bool exist = search(name, type::file);
	if (!exist)
	{
		throw objectNotFound();
	}
	...
	uShort charsToRead = fileSector[0];
	for (int i = 0; i < 5; i++)
	{
		currentFile->filename[i] = name[i];
	}
	currentFile->fileContent.resize(charsToRead);
	currentFile->fileSize = charsToRead;
	currentFile->sizeInSectors = row[6];
...
void ChaOS_filesystem::saveFile()
{
	...
	while (currentFileSector && charsToWrite)
	{
		...
		std::cout << "save-sizeSector:" << currentFile->sizeInSectors << std::endl;
		disk.readSector(currentFileSector, fileSector);
		...
		if (charsToWrite > 0 && fileSector[31] == 0)
		{
	...
	char name[5]; toChar5(name, currentFile->filename);
	char row[8] = {};
	char dirSector[32] = {};
	...
	while (currentDirSector)
	{
		...
		for (int i = 0, j = 0; i < 3; i++, j += 8)
		{
			...
			getRow(&dirSector[j], row);
			for (int i = 0; i < 8; i++) std::cout <<" "<<short(row[i]) << std::flush;
			std::cout << std::endl;
			if (equalName(name, row))
			{
...
disk_drive & ChaOS_filesystem::getDisk()
{
	return disk;
}
...
bool bitMask::getBit(unsigned int INT, unsigned int i)
{
	return INT & (1 << i);
}

unsigned int bitMask::setBit(const unsigned int INT, unsigned int i, bool value)
{
...
objectNotFound::objectNotFound()
	: exception("Cannot find selected object!")
{
}
